...
function generate() {
    ...
    block += "<tr>";
    //Prev WEEK
    block += "<th>ID</th><th>PW: Available Tasks</th><th>PW:Accepted Tasks</th><th>PW:Canceled T.</th><th>PW: Concluded T.</th><th>PW: Times Acessed Platform</th><th>PW: Time spent online</th><th>PW: avg days to conclude T.</th><th>PW: leader previous week?</th><th>PW: leader task Concluded?</th>";
    //THIS WEEK
    block += "<th>TW: Available Tasks</th><th>TW: Accepted Tasks</th><th>TW:Canceled T.</th><th>TW: Concluded T.</th><th>TW: Times Acessed Platform</th><th>TW: Time spent online</th><th>TW: avg days to conclude T.</th><th>TW: leader previous week?</th><th>TW: leader task Concluded?</th></tr>";
    ...
    for (let i = 1; i < numOfEachPersona + 1; i++) {
        block += "<tr>";
...
function populateTable(availableTask, acceptedtask, canceledtask, concludedTask, timesAccessedPlatform, timeSpentOnThePlatform, avgDaysToConcludeTask) {
    ...
    block += "<td>" + thisTimesAccessed + " times</td>"; // Times accessed platform
    block += "<td>" + getRandomIntInclusiveCeil(timeSpentOnThePlatform * timesAccessedPlatform - 30, timeSpentOnThePlatform * timesAccessedPlatform + 300) + " seconds</td>"; // time spent per access on avg
    block += "<td>" + getRandomIntInclusiveCeil(avgDaysToConcludeTask - 1, avgDaysToConcludeTask + 1) + " days</td>"; // avg day to conclude a task
